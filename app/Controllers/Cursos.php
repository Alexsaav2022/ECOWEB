        ...
        foreach ($registro as $key => $value) {
            if (array_key_exists('Authorization', $headers) && !empty($headers['Authorization'])) {
                ...
                if ($request->getHeader('Authorization') == 'Authorization: Basic ' . base64_encode($value['cliente_id'] . ':' . $value['llave_secreta'])) {
                    ...
                    $curso = $model->getCursos();
                    //var_dump($curso); die;
                    if (!empty($curso)) {
                        $data = array(
                            "Status" => 200,
                            "Total de registros"=>count($curso),
                            "Detalles" => $curso
                        );
                    } else {
                        $data = array(
            ...
            if (array_key_exists('Authorization', $headers) && !empty($headers['Authorization'])) {
                if ($request->getHeader('Authorization') == 'Authorization: Basic ' . base64_encode($value['cliente_id'] . ':' . $value['llave_secreta'])) {
                    $datos = array(
                        ...
                        "id_tipo" => $request->getVar("id_tipo"),
                        "id_categoria" => $request->getVar("id_categoria"),
                        "id_naturaleza" => $request->getVar("id_naturaleza")
                    );
                    if (!empty($datos)) {
                        $validation->setRules([
                        ...
                        if ($validation->getErrors()) {
                            ...
                            $datos = array(
                                ...
                                "id_naturaleza" => $datos["id_naturaleza"]
                            );
                            $model = new CursosModel();
